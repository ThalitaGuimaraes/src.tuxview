...
export class DatabaseService {
  ...
  //Vou contratar um tradutor - httpsOptions
  httpOptions = {
  ...
  postFoto(dados: Fotos){
    this.http.post('http://localhost:3000/fotos', JSON.stringify(dados), this.httpOptions).subscribe();
  }

  //Exclusão da foto
  delFoto(id:number){
    return this.http.delete('http://localhost:3000/fotos/' + id).subscribe();
  }
}
